...
class MainWindow(QOpenGLWindow):
    ...
    def createFramebufferObject(self):
        ...
        glRenderbufferStorage(
            GL_RENDERBUFFER, GL_DEPTH_COMPONENT, self.textureWidth, self.textureHeight
        )
        # bind
        ...
        glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, self.textureID, 0
        )

        # now attach a renderbuffer to depth attachment point
        glFramebufferRenderbuffer(
            GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, rboID
        )
