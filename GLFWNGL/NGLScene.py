...
class NGLScene:
    ...
    def mousePressEvent(self, button, x, y):
    ...
    def mouseMoveEvent(self, button, x, y):
        if self.rotate and button == glfw.MOUSE_BUTTON_LEFT:

            diffx = int(x - self.origX)
            diffy = int(y - self.origY)
            self.spinXFace += int(0.5 * diffy)
            self.spinYFace += int(0.5 * diffx)
            self.origX = x
            self.origY = y
        elif self.translate and button == glfw.MOUSE_BUTTON_RIGHT:

            diffX = int(x - self.origXPos)
            diffY = int(y - self.origYPos)
            self.origXPos = x
            self.origYPos = y
            self.modelPos.m_x += self.INCREMENT * diffX
            self.modelPos.m_y -= self.INCREMENT * diffY

    def wheelEvent(self, x, y):
        print("wheel ", x, y)
        if y > 0:
